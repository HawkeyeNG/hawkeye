...
| Deplatforming / single-host failure | Provider-portable AI (base-URL swap) and a planned **self-owned failover cluster** (2× NVIDIA DGX Spark + Starlink + 10 kVA / 30 kWh solar, sized to back up 1M users) that can host the full stack + local models off-grid — availability independent of any one cloud, ISP, or grid |
