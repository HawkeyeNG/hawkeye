...
| Uploading an old/downloaded photo | The PWA has **no file picker** — photos come only from live `getUserMedia` camera captures. The server rejects any photo whose capture timestamp is in the future or older than `PHOTO_LATE_MAX_S` (24 h). Between `PHOTO_MAX_AGE_S` (1 h) and that, a report held offline is accepted but marked **late** — timing not server-verified — and late reports alone cannot make a unit verified. |
