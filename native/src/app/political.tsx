  ...
  return (
    <SafeAreaView className="flex-1 bg-hawk-mist">
      <View className="flex-row items-center px-4 pt-2">
        ...
        <Text className="pl-3 text-lg font-bold text-hawk-ink">Political Data</Text>
      </View>
      ...
      <ScrollView contentContainerClassName="px-4 pb-10 pt-3">
        ...
        {err ? (
          ...
          <>
            {d.president ? (
              <View className="flex-row items-center rounded-2xl bg-white px-4 py-3">
                ...
                <View className="flex-1 pl-3">
                  <Text className="text-sm font-bold text-hawk-ink">{d.president.name}</Text>
            ...
            <Pressable
              className="mt-3 flex-row items-center rounded-2xl bg-white px-4 py-3.5 active:opacity-80"
              onPress={() => router.push('/candidates')}
            >
              <Feather name="users" size={17} color={BRAND.leaf} />
              <Text className="flex-1 pl-3 text-sm font-semibold text-hawk-ink">
            ...
            {d.composition ? (
              <>
                ...
                {Object.entries(d.composition.chambers).map(([key, ch]) => {
                  ...
                  return (
                    <View key={key} className="mb-2 rounded-2xl bg-white px-4 py-3">
                      <View className="flex-row items-baseline">
                        <Text className="flex-1 text-sm font-bold text-hawk-ink">{ch.label}</Text>
                        <Text className="text-[11px] text-neutral-400">
                          {ch.total}/{ch.size} seats declared
                        </Text>
                      </View>
                      <View className="mt-2 h-2.5 flex-row overflow-hidden rounded-full bg-hawk-mist">
                        {parties.map(([p, n]) => (
                          <View
                            key={p}
                            style={{
                      ...
                      <View className="flex-row flex-wrap pt-2">
                        {parties.map(([p, n]) => (
                          <View key={p} className="mr-3 flex-row items-center pt-1">
                            ...
                            />
                            <Text className="pl-1 text-[11px] text-neutral-600">
                              {p} {n}
                            </Text>
                          </View>
                        ))}
                ...
                {d.composition.note ? (
                  <Text className="pt-1 text-xs text-neutral-400">
                    {d.composition.note}
            ...
            {byParty.groups.length ? (
              <>
                ...
                {byParty.groups.map(([party, sts]) => (
                  <View key={party} className="mb-2 rounded-2xl bg-white px-4 py-3">
                    ...
                    <Text className="pt-1.5 text-xs text-neutral-600">
                      {sts.sort().join(' · ')}
                    </Text>
                  </View>
                ))}
              ...
              <>
                ...
                <View className="overflow-hidden rounded-2xl bg-white">
                  {d.upcoming.elections.map((e, i) => (
                    <View
                      key={e.office}
                      className={`px-4 py-3 ${i > 0 ? 'border-t border-hawk-mist' : ''}`}
                    >
                      <View className="flex-row items-baseline">
                        <Text className="flex-1 text-sm font-bold text-hawk-ink">{e.office}</Text>
                ...
                {d.upcoming.note ? (
                  <Text className="pt-2 text-xs text-neutral-400">{d.upcoming.note}</Text>
                ) : null}
              </>
            ) : null}

            {d.note ? (
              <Text className="pt-4 text-xs text-neutral-400">{d.note}</Text>
            ) : null}
          </>
        )}
      </ScrollView>
