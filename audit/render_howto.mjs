...
const CLIPS = [
  ...
  },
  {
    ...
    steps: [
      { cap: 'Open the Leaderboard — it builds live as reports come in.',
        screen: scr('Leaderboard', h1('Leaderboard') + card(results([['Party A', '41,203'], ['Party B', '33,881'], ['Party C', '12,406']])) + `<div class="ph-fine">Placeholder parties shown. Unofficial — INEC declares the official result.</div>`) },
      { cap: 'Pick the election and the area you care about.',
        screen: scr('Leaderboard', h1('Choose your race') + card(label('Election') + select('Osun Governorship', true) + label('Area') + select('Osun'))) },
      { cap: 'Tap Follow — verify your phone once, then get Telegram alerts.',
        screen: scr('Leaderboard', card(btn('Follow', { tap: true, icon: IC_BELL })) + ok('Following Osun', "You'll get a Telegram message on every new report.")) },
