...
WHAT YOU CAN DO
• Report a polling-unit result — photograph the result sheet and enter the counts. On-device scanning flattens the sheet and reads the numbers to help you fill them in quickly.
...
**The scanning sentence belongs here, and used to be missing.** This file said
it was dropped "on purpose" because `@capacitor-mlkit/document-scanner` is
Android-only — but that package belongs to `mobile/`, the retired Capacitor app.
The app being submitted is `native/`, which uses
`react-native-document-scanner-plugin` 2.0.4, and that ships a full iOS
implementation (`ios/DocumentScanner.swift`, `ios/DocScanner/`). Our own
patch-package patch modifies BOTH the Android Kotlin and the iOS Swift, there is
no `Platform.OS` gate anywhere in `lib/scan.ts`, and the scanner was confirmed
working on an iPhone from TestFlight build 7.

So the iOS listing was understating the app because of a note about a different
binary. Text recognition is likewise present on both platforms
(`@react-native-ml-kit/text-recognition`), with a server-side OCR cross-check
...
**Do NOT use `app/ios-shots/`.** Those are captures of the WEBSITE — home, how,
integrity, faq — taken by `scripts/ios_screenshots.mjs` back when the iOS app was
the Capacitor wrapper. They show pages the submitted binary does not contain, and
Guideline 2.3.3 is about exactly that.

The real set comes from the same pipeline Play uses, so the two listings cannot
drift:

```bash
# 1. Metro, serving the RN app
cd native && EXPO_PUBLIC_API_BASE=http://localhost:8430 npx expo start --web --port 8092
# 2. capture the raw frames from the app itself
cd tests/ui && ./run_capture.sh
# 3. caption and size them, once per store
cd backend
node scripts/make_store_screenshots.mjs --in /tmp/raw --out <out>/play-phone
node scripts/make_store_screenshots.mjs --in /tmp/raw --out <out>/ios-6.9 --w 1320 --h 2868
```

- Captured at **440x956 CSS, deviceScaleFactor 3, isMobile** — the iPhone 16 Pro
  Max logical size, so the layout is a phone's and not a tablet's blown up.
- **Dark**, because the theme preference ships as `system` and dark is what
  someone opening the app actually sees. The capture used to hardcode
  `colorScheme: 'light'`.
- Captioned in the style `make_store_screenshots.mjs` documents: brand-leaf
  background, two-line caption, device bled off the bottom edge.
- **6.9" 1320 x 2868** for Apple, **1080 x 1920** for Play. One design, scaled;
  the canvas is an argument rather than a second script.

`1-capture` and `2-figures` are still to shoot — both need a printed specimen
EC8A in front of a real camera, which no headless run can provide. Generate the
sheet with `make_specimen_ec8a.mjs`; never photograph a real EC8A, which carries
a real unit's real votes.
