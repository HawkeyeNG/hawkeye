...
observersRouter.post('/register', async (req, res) => {
  ...
  const body = { ok: true };
  // Sendchamp generated + delivered this code (WhatsApp or primary SMS);
  // /verify must confirm against their reference instead of our local code.
  if (sent.scReference) {
...
observersRouter.post('/verify', async (req, res) => {
  ...
  // Sendchamp-generated codes (WhatsApp, or SMS via the primary provider) are
  // confirmed against their reference; every other channel checks our locally
  // stored code. The local attempts counter and expiry apply to both paths.
